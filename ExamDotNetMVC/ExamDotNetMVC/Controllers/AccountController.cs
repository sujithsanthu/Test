...
namespace ExamDotNetMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                {
                    ...
                    {
                        //await Login(model.RememberMe,);
                        return RedirectToAction("Index", "Admin");
                    }
                    else
                    {
                        return RedirectToAction("ExamDetails", "WebUsers");
                    }
                }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model,int Experience)
        {
            if (Experience == 0)
            {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var ran = new Random();
                    var questions = db.AddQuestions.Where(w => w.QLevel.Equals(Experience)).Take(10).ToList()
                        .OrderBy(q => ran.Next()).ToList();
                    foreach (var question in questions)
                    {
                        ExamDetail exam = new ExamDetail();
                        exam.Name = model.Email;
                    ...
                    userDetail.Name = model.Email;
                    userDetail.Status = "UnChecked";
                    userDetail.Experience = Experience;
                    userDetail.Email = model.Email;
                    userDetail.Phone = model.PhoneNumber;             
